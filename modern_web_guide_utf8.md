...
| :--- | :--- | :--- | :--- |
| **1ΓÇô5** | Single (Exclusive) | `<input type="radio">` | **Zero-click scanning**: All choices are immediately visible. Faster scan time. |
...
- **DO** hide inputs visually using the canonical `.visually-hidden` recipe (`clip-path: inset(50%)` with 1px dimensions) ΓÇö NOT `display: none`, which removes them from the accessibility tree.
